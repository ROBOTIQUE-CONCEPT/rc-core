...
## 0.6.0-alpha12
- **Reverted 0.6.0-alpha11's removal.** Restored `Contracts\ModuleInterface`, `Module\ModuleRegistry`, `rc_register_module()`, the `wprc/core/register_modules` action, and `Plugin::modules()`/its container registration, exactly as they were before alpha11. The removal broke production: RC Core is network-active, and **RC-Catalog** (the separate `www`-side plugin, outside this repository and outside the scope of the 2026-09-19 documentation audit that recommended the removal) depends on this mechanism — it was never checked, because its source isn't in this repo, `rc-portal`, or `rc-portal-theme`. `docs/ARCHITECTURE-OPEN-QUESTIONS.md`'s entry on the two module-lifecycle contracts is reopened; see that file for the corrected options.

## 0.6.0-alpha11 (superseded by alpha12 — do not deploy)
- **Removed** Core's own module-lifecycle mechanism: `Contracts\ModuleInterface`, `Module\ModuleRegistry`, the `rc_register_module()` function, and the `wprc/core/register_modules` action (fired from `Plugin::boot()`), along with `Plugin::modules()` and its container registration. Architecture decision 2026-09-19, resolving `docs/ARCHITECTURE-OPEN-QUESTIONS.md`'s entry on two incompatible module-lifecycle contracts: this mechanism was fully built but had zero callers anywhere across `rc-core`, `rc-portal`, or `rc-portal-theme` (confirmed by repo-wide search before removal) — `rc-portal`'s embedded modules have always used their own `EmbeddedModuleInterface`/`ModuleCatalog` instead. **This check missed RC-Catalog, a fourth, separate plugin not covered by that search — it turned out to depend on this mechanism, which broke production. Reverted in alpha12; do not deploy this version.**
- **Removed** Core's own module-lifecycle mechanism: `Contracts\ModuleInterface`, `Module\ModuleRegistry`, the `rc_register_module()` function, and the `wprc/core/register_modules` action (fired from `Plugin::boot()`), along with `Plugin::modules()` and its container registration. Architecture decision 2026-09-19, resolving `docs/ARCHITECTURE-OPEN-QUESTIONS.md`'s entry on two incompatible module-lifecycle contracts: this mechanism was fully built but had zero callers anywhere across `rc-core`, `rc-portal`, or `rc-portal-theme` (confirmed by repo-wide search before removal) — `rc-portal`'s embedded modules have always used their own `EmbeddedModuleInterface`/`ModuleCatalog` instead. **Compatibility:** breaking for any external code implementing `Contracts\ModuleInterface` or calling `rc_register_module()`/listening for `wprc/core/register_modules` — none exists in this platform's own repos today. `RC\Portal\Module\EmbeddedModuleInterface` (in `rc-portal`) is unaffected and remains the real, canonical module-lifecycle contract. See `docs/MODULE-DEVELOPMENT.md` and `docs/ARCHITECTURE.md`'s Amendments section (decision 3).
